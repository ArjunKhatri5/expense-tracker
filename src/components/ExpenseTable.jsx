...
function ExpenseTable({expenses, setExpenses, setFormData, setEditingRowId }) {
  ...
  const totalAmount = filteredData.reduce((accumulatore, current) => accumulatore + parseFloat(current.amount), 0)

      
  return (
    <>
    <ContextMenu menuPosition={menuPosition} setMenuPosition={setMenuPosition} setExpenses={setExpenses} rowId={rowId} setFormData={setFormData} expenses={expenses} setEditingRowId={setEditingRowId} />
    <table className="expense-table" onClick={() => {
      ...
      <tbody>
        {filteredData.sort(sortCallback).map((exp) => {
          return (
